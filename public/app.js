            ...
            container.innerHTML = `
                ...
                <div class="profile-header">
                    ...
                    <div class="profile-badges" style="display:flex; gap:8px; justify-content:center; align-items:center;">
                        <span class="c-badge ${roleBadgeClass}" style="font-size:11px; padding:6px 12px;">${roleName}</span>
                        <span class="c-badge ${lvlClass}" style="font-size:11px; padding:6px 12px;"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style="margin-right:4px;"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg> Lvl. ${level}</span>
                        <span class="c-badge" style="font-size:11px; padding:6px 12px; background: rgba(255,255,255,0.05); color: #a1a1aa; border: 1px solid rgba(255,255,255,0.1);">${shortUid}</span>
                    </div>
                </div>
...
function getHighRes(url) {
    if(!url) return '';
    try { return url.replace(/\/s\d+(-[a-zA-Z0-9]+)?\//g, '/s0/').replace(/=s\d+/g, '=s0').replace(/\?resize=\d+,\d+/g, ''); } catch(e) { return url; }
}
...
function addXP(amount) {
    ...
    userRef.once('value').then(snap => {
        ...
        let oldExp = data.exp || 0; let oldLvl = data.level || 1;
        let newExp = oldExp + amount; let newLvl = Math.floor(newExp / 200) + 1; 
        
        userRef.update({ exp: newExp, level: newLvl });
        const toast = document.getElementById('xp-toast');
        const toastText = document.getElementById('xp-toast-text');
        
        if (newLvl > oldLvl) { toastText.innerText = `Level Up! Level ${newLvl} 🎉`; toast.style.background = '#f59e0b'; } 
        else { toastText.innerText = `+${amount} XP`; toast.style.background = '#3b82f6'; }
        
        toast.style.display = 'flex'; setTimeout(() => { toast.style.display = 'none'; }, 3000);
    });
}

function initDB() { return new Promise((resolve, reject) => { const req = indexedDB.open(DB_NAME, 2); req.onupgradeneeded = (e) => { const database = e.target.result; if (!database.objectStoreNames.contains(STORE_HISTORY)) database.createObjectStore(STORE_HISTORY, { keyPath: 'url' }); if (!database.objectStoreNames.contains(STORE_FAV)) database.createObjectStore(STORE_FAV, { keyPath: 'url' }); }; req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); }); }
async function saveHistory(animeObj) { try { const database = await initDB(); const tx = database.transaction(STORE_HISTORY, 'readwrite'); animeObj.timestamp = Date.now(); tx.objectStore(STORE_HISTORY).put(animeObj); } catch(e) {} }
async function getHistory() { try { const database = await initDB(); return new Promise((resolve) => { const req = database.transaction(STORE_HISTORY, 'readonly').objectStore(STORE_HISTORY).getAll(); req.onsuccess = () => resolve(req.result.sort((a,b) => b.timestamp - a.timestamp)); }); } catch(e) { return []; } }
async function toggleFavorite(url, title, image, score, episode) { try { const database = await initDB(); const isFav = await checkFavorite(url); const tx = database.transaction(STORE_FAV, 'readwrite'); const store = tx.objectStore(STORE_FAV); const btn = document.getElementById('favBtn'); if (isFav) { store.delete(url); if(btn) { btn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg> Subscribe`; btn.style.color = '#fff'; } } else { store.put({url, title, image, score, episode, timestamp: Date.now()}); if(btn) { btn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="#ef4444" stroke="#ef4444" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg> Disubscribe`; btn.style.color = '#ef4444'; } } } catch(e) {} }
...
async function getFavorites() { try { const database = await initDB(); return new Promise((resolve) => { const req = database.transaction(STORE_FAV, 'readonly').objectStore(STORE_FAV).getAll(); req.onsuccess = () => resolve(req.result.sort((a,b) => b.timestamp - a.timestamp)); }); } catch(e) { return []; } }
...
// ==== FUNGSI LOAD LATEST (HOME - ORIGINAL) ====
...
// ==== FUNGSI TIMELINE HISTORY (FULL WIDTH) ====
async function loadRecentHistory() {
            ...
            timelineHtml += `
                <div class="timeline-card" onclick="loadDetail('${anime.url}')">
                    <div class="timeline-img">
                        ...
                        <div class="timeline-play-icon"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></div>
                    </div>
    ...
    timelineHtml += '</div>'; container.innerHTML = timelineHtml;
}
...
// ==========================================
// 8. WATCH VIEW & COMMENTS SYSTEM (PREMIUM)
// ==========================================
        ...
        document.getElementById('watch-view').innerHTML = `
            ...
            <div style="padding: 15px 20px; display: flex; gap: 12px; align-items: center; border-bottom: 1px solid #111;">
                <div style="flex: 1;">
                    ...
                    <div style="font-size: 12px; color: #a1a1aa; font-weight: 500;">Episode ${currentEpNum} • ${mockViews} • ${mockDate}</div>
                </div>
            </div>
...
window.setCommentFilter = function(sortType, btnElement) {
    ...
    if (window.currentEpID) listenToComments(window.currentEpID); // Refresh list saat difilter
}
...
function listenToComments(epID) {
    ...
    db.ref('comments/' + epID).on('value', snap => {
        ...
        // Update jumlah komentar dinamis
        if(countEl) {
            let total = commentsArr.length;
            countEl.innerText = total > 1000 ? (total/1000).toFixed(1) + 'K Comments' : total + ' Comments';
        }

        // Logic Filter Komentar
        if(window.currentCommentSort === 'new') {
            commentsArr.sort((a, b) => b.waktu - a.waktu); 
        } else {
            commentsArr.sort((a, b) => a.waktu - b.waktu); 
        }
