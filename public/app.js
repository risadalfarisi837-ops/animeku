...
async function loadVideo(url) {
    ...
    try {
        ...
        let displayTitle = window.currentAnimeMeta?.title || data?.title || 'Membuka Episode...';
        ...
        if(window.currentAnimeEpisodes && window.currentAnimeEpisodes.length > 0) { 
            ...
            if(foundEp) { 
                let epMatch = String(foundEp.title).match(/(?:Episode|Eps|Ep)\s*(\d+(\.\d+)?)/i); 
                currentEpNum = epMatch ? epMatch[1] : (String(foundEp.title).match(/\d+/g) ? String(foundEp.title).match(/\d+/g).pop() : "1"); 
            } 
        }
        ...
        // ANTI-CRASH: Cek apakah data streams benar-benar ada dari server
        let streams = data?.streams || [];
        let initialServer = streams.length > 0 ? streams[0].server : ''; 
        let initQualMatch = initialServer ? initialServer.match(/\d{3,4}p/i) : null; 
        let displayQualText = initQualMatch ? initQualMatch[0] + ' Quality' : 'Quality';
        let videoIframe = streams.length > 0 ? `<iframe id="video-player" src="${streams[0].url}" allowfullscreen></iframe>` : `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:#ef4444; font-weight:bold; background:#111; font-size:13px;">Video Belum Tersedia / Error Server</div>`;

        document.getElementById('watch-view').innerHTML = `
            <div class="video-container-fixed"><button class="watch-back-btn" onclick="backToDetail()"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>${videoIframe}</div>
        ...
        if (streams.length > 0) { 
            ...
            document.getElementById('modal-server-list').innerHTML = `<div style="text-align:center; color:#888; padding:20px;">Video tidak memiliki opsi server alternatif.</div>`;
        }
        ...
        window.showToast("Gagal merender tampilan video.", "error"); 
...
window.buyItem = function(cat, id, harga) {
    ...
    let updateData = { koin: koin - harga }; updateData[ownedKey] = true;
    db.ref('users/' + currentUser.uid).update(updateData).then(() => { window.showToast('Berhasil membeli!', 'success'); });
};
...
window.previewGiftUser = function(val) {
    let uidShort = val.replace('#', '').trim().toUpperCase(); let previewDiv = document.getElementById('gift-user-preview');
    if (!uidShort || uidShort.length !== 6) { previewDiv.style.display = 'none'; return; }
    db.ref('users').once('value').then(snap => {
        ...
        if (targetData) {
            ...
            if (currentUser && fullUid === currentUser.uid) { document.getElementById('gift-preview-name').innerText = targetData.nama + ' (Kamu)'; previewDiv.style.borderColor = '#ef4444'; } 
            else { document.getElementById('gift-preview-name').innerText = targetData.nama; previewDiv.style.borderColor = '#3b82f6'; }
