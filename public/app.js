...
// ==== FUNGSI BANTUAN TANGGAL TIMELINE & WAKTU KOMENTAR ====
...
// ==== FUNGSI LOAD LATEST (ANTI MUTER/LOADING TERUS) ====
async function loadLatest() {
    ...
    try {
        ...
        loader(false); // Pastikan loading spinner selalu dimatikan
    }
}
    ...
    try {
        ...
        document.getElementById('watch-view').innerHTML = `
            ...
            <div class="comment-section" style="padding: 20px;">
                <div style="font-size:16px; font-weight:800; margin:0 0 15px 0;">5,6K Comments</div>
                <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                    <button class="comment-filter-btn active">Top Comment</button>
                    <button class="comment-filter-btn">Terbaru</button>
                </div>
    ...
    if(!currentUser) {
        container.innerHTML = `
            <div style="display: flex; gap: 12px; align-items: center;">
                <div style="width: 36px; height: 36px; border-radius: 50%; background: #222; display: flex; align-items:center; justify-content:center;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#555"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
                </div>
                <div style="flex: 1; background: #1c1c1e; border: 1px solid #2c2c2e; padding: 10px 16px; border-radius: 24px; color: #888; font-size: 13px; cursor: pointer;" onclick="switchTab('developer')">
        ...
        db.ref('users/' + currentUser.uid).once('value').then(snap => {
            const u = snap.val();
            const userFoto = u?.foto || 'https://placehold.co/40';
            container.innerHTML = `
                <div style="display: flex; gap: 12px; align-items: center;">
                    <img src="${userFoto}" style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover;">
                    <div style="flex: 1; position: relative;">
                        ...
                        <button onclick="postComment('${epID}')" style="position: absolute; right: 6px; top: 50%; transform: translateY(-50%); background: transparent; color: #3b82f6; border: none; padding: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer;">
...
function listenToComments(epID) {
    ...
    db.ref('comments/' + epID).on('value', snap => {
        ...
        snap.forEach(child => {
            ...
            // Logika Warna Badge Level
            let lvlClass = 'badge-lvl-normal';
            if (level >= 100) lvlClass = 'badge-lvl-gold';
            else if (level >= 50) lvlClass = 'badge-lvl-silver';

            html = `
                <div class="comment-item" style="display: flex; gap: 12px; margin-bottom: 25px;">
                    <img src="${c.foto}" style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover; flex-shrink: 0; margin-top: 4px;">
                    <div style="flex: 1; min-width: 0;">
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 2px;">
                            <span style="font-weight: 700; font-size: 13px; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${c.nama}</span>
                        ...
                        <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 6px; flex-wrap: wrap;">
                            ...
                            <span style="font-size: 11px; color: #666; font-family: monospace; letter-spacing: 0.5px;">${uidStr}</span>
                        </div>
                        <div style="font-size: 13px; color: #d1d5db; line-height: 1.5; margin-bottom: 8px; word-wrap: break-word;">${c.teks}</div>
                        <div style="font-size: 12px; color: #3b82f6; font-weight: 700; cursor: pointer;">Reply</div>
                    </div>
                </div>
            ` + html;
        });
